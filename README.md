# MaiQRCodeGetter
用来让你免打开微信公众号，直接通过浏览器获取到舞萌（MaiMai DX / 中二节奏）登录二维码，
让你无手机出勤（借手机嘛，起码不用登录微信，或者没电了续命，或者给家里打舞萌的小孩用），或者...单片机出勤可能！
...
> 使用 SakuraFrp 同省节点速度，窗口非前台窗口且未被最小化，完成后不最小化窗口
...
# 监听端口 (1-65535)
port = 8080
# 运行模式，normal为正常模式，marked会隐藏敏感信息，demo会替换敏感信息，web_only只生成演示二维码
mode = marked
```

对于使用 `demo` 和 `web_only` 模式产生的二维码可随意发放，作为演示使用，此模式产生的二维码基于时间或原二维码哈希后生成，无法用于实际登录。

1.打开微信，找到”舞萌 | 中二”公众号

2.双击侧边栏分离公众号窗口，点击一次获取玩家二维码

3.使用手机扫描或截图识别二维码是否能够识别，如果不能识别请按照下方图片设置（微信版本≥4.0.0）
...
默认用户名`admin`，密码`maimaidx`，登录成功后会进行一次获取二维码，获取成功则显示二维码和过期时间。
登录支持多设备登录，重启程序后销毁
...
如果您使用他人设备出勤却忘记退出账号且无法操控程序关闭，可以寻找任意设备登录后访问 `/logout_all` 
强制使全部登录失效（包括此设备）
...
1. [青葉もち](https://www.pixiv.net/users/27236214) - 使用了其创作的图片作为网页背景

如果您不愿意您的资源被本项目使用，请通过Issue联系我进行删除。

# 杂谈
本项目专注于通过电脑端微信自动获取舞萌公众号的登录二维码，让你无需手机即可出勤。

项目到这里基本没有什么能够更新的了，目前完全没有无头客户端无法实现免打扰获取二维码，
所以本程序采用系统API以快速获取前置窗口和目标窗口实现点击，
采用BitBit截图以实现遮盖截图，实际窗口显示时间可以达到0.2秒以内，
尽量减少干扰，整体获取时间可达2.2秒内，又推出了最小化窗口让其不会被意外操作关闭，
且现版本微信客户端可以实现消息放大使二维码图片放大，故截屏方法可用性极高，
而通过代理操作获取二维码链接较为复杂且需要安装额外软件，因此目前此方法已经算是最优解了。
